...
try:
    ...
    print(f"[ERROR] BaiduPCS-Py未安装: {e}")
...
try:
    ...
    if results:
        print(f"[OK] 下载完成，共 {len(results)} 个文件:")
        for i, result in enumerate(results, 1):
            if result.success:
                print(f"  {i}. [OK] {result.local_path}")
            else:
                print(f"  {i}. [ERROR] {result.remote_path}: {result.error}")
    else:
        print("[ERROR] 下载失败")
